...
Status: Active
Updated: 2026-07-27
Scope: Roadmap and discovery only. No business logic, database, migration, or payment behavior changes are included in this document.
...
- `docs/testing-database-safety-standard.md`
- `docs/multi-language-standard.md`
...
- `docs/backend-ui-standardization-roadmap.md`
...
Pricing logic needs a deeper audit because calculation appears spread across controller paths and service classes. `HotelPricingService` exists, but booking creation paths must be verified to ensure it is the single source of truth.
...
- `ACC-PRICE-001` Compare price calculation across `HotelsController`, `OrderController`, and `HotelPricingService`.
- `ACC-PRICE-002` Document all price inputs and rounding/currency/tax rules.
...
- `ACC-SEC-001` Audit authorization and IDOR protection for order detail, edit, invoice, and payment confirmation.
...
- Repeat end-to-end audit pattern for other service flows.
...
- Transport, tour, and activity have service-specific pricing/inventory services and order paths.
- Wedding and restaurant may have wider operational dependencies and should be audited after the core booking baseline is stable.
...
## 10. Safest First Implementation Task

Recommended first task after this roadmap:

`ACC-STATUS-001 - Build a read-only Accommodation status and lifecycle inventory`

Reason:

- Status inconsistency is high risk across order display, admin review, payment, and history.
- It can begin with read-only code and data audit.
- It has low dependency on pricing or UI decisions.
- It reduces risk before touching order creation, invoice, or payment behavior.

Expected output:

- Status lifecycle table for Accommodation.
- List of all code locations that write each status.
- List of all code locations that filter each status.
- Read-only database status inventory.
- Recommendation for whether to preserve, alias, migrate, or deprecate legacy status values.

No status normalization, migration, or data update should happen inside this first task.
