...
@section('content')
                    ...
                    <div class="col-md-8">
                        <div class="card-box">
                            ...
                            @if(count($optional_rate_orders)>0)
                                <form id="updateOrderAdditionalCharge" action="{{ route('func.update.order-additional-charge',$order->id) }}" method="post" enctype="multipart/form-data">
                                    @csrf
                                    @method('put')
                            @else
                                <form id="updateOrderAdditionalCharge" action="{{ route('func.create.order-additional-charge',$order->id) }}" method="post" enctype="multipart/form-data">
                                    @csrf
                            @endif
                                <div class="row">
                                    <div class="col-md-12">
                                                                ...
                                                                <div class="row">
                                                                    ...
                                                                    <div class="col-sm-4">
                                                                        <div class="form-group">
                                                                            ...
                                                                            <select {{ $optional_rate_order->mandatory == 1?"readonly":""; }} name="optional_rate_id[]" class="custom-select" required>
                                                                                @if ($optional_rate_order->mandatory == 1)
    ...
    @include('partials.loading-form', ['id' => 'updateOrderAdditionalCharge'])
    ...
    <script>
        $(document).on('click', '.btnremove', function() {
            ...
            if (confirm('Are you sure you want to delete this booking?')) {
                $.ajax({
                    url: '/fdelete-order-additional-charge/' + orderHotelPromoDetailId,
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    }
