    ...
    @extends('layouts.head')
                    ...
                    <div class="row">
                        <div class="col-xl-8">
                            <div class="card-box">
                                ...
                                <hr class="form-hr">
                                <form id="create-order" action="{{ route('func.create.order-hotel-promo') }}" method="POST">
                                    @csrf
                                    <div class="row">
                                        <div class="col-md-6">
                                                        ...
                                                        <div class="row">
                                                            ...
                                                            <div class="col-sm-3">
                                                                <div class="form-group">
                                                                    ...
                                                                    <input id="number_of_guests[]" type="number" min="1" max="{{ $room_capacity }}" name="number_of_guests[]" class="guest-input form-control m-0 @error('number_of_guests[]') is-invalid @enderror" placeholder="{{ __('messages.Capacity') ." ".$room->capacity." ". __('messages.guests') }}" value="{{ old('number_of_guests[]') }}" required>
                                    ...
                                    @if ($totalPriceOptionalRates >0)
                                        <div class="page-subtitle">@lang('messages.Additional Charge')</div>
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="row">
                                                    <div class="col-sm-12">
                                                            ...
                                                            <tbody>
                                                                @foreach ($optional_rates as $optional_rate)
                                                                    <tr class="optional-rate-row">
                                                                        <td>
                                                                            <div class="table-service-name">{{ dateFormat($optional_rate->active_date) }}</div>
                                                                        </td>
                                                                        <td>
                                                                            <div class="number_of_guests table-service-name guest-total-display">0</div>
                                                                        </td>
                                                                        <td>
                                                                            <div class="table-service-name">{{ $optional_rate->name }}</div>
                                                                        </td>
                                                                        <td>
                                                                            <div data-price-pax="{{ $optional_rate->calculatePrice($usdrates, $tax) }}" class="price-per-pax table-service-name">{{ currencyFormatUsd($optional_rate->calculatePrice($usdrates, $tax)) }}/@lang('messages.pax')</div>
                                                                        </td>
                                                                        <td>
                                                                            <div class="total-price-optional-rate table-service-name">{{ currencyFormatUsd(0) }}</div>
                                                                        </td>
                                                                    </tr>
                                                                @endforeach
                                                            </tbody>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function () {
                var currencyFormatter = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD',minimumFractionDigits: 2, maximumFractionDigits: 2  });
                ...
                var grandAdditionalChargePrice = 0;
                ...
                function updateTotalGuests() {
                    ...
                    // Hitung total semua guests
                    document.querySelectorAll('input[name="number_of_guests[]"]').forEach(input => {
                    ...
                    // Update ke semua elemen guest display
                    document.querySelectorAll('.guest-total-display').forEach(el => {
                        el.textContent = totalGuests;
                    });
                    let grandTotal = 0;

                    // Update total harga per baris optional rate
                    document.querySelectorAll('tr').forEach(row => {
                        const pricePerPaxEl = row.querySelector('.price-per-pax');
                        const totalPriceEl = row.querySelector('.total-price-optional-rate');

                        if (pricePerPaxEl && totalPriceEl) {
                            const pricePerPax = parseFloat(pricePerPaxEl.dataset.pricePax);
                            if (!isNaN(pricePerPax)) {
                                const totalPrice = totalGuests * pricePerPax;

                                // Format angka jadi mata uang (misalnya USD)
                    ...
                    const totalChargeEl = document.getElementById('totalAdditionalCharge');
                    if (totalChargeEl) {
                        totalChargeEl.textContent = '$' + grandTotal.toFixed(2);
                    }
                    
                    const totalChargePriceEl = document.getElementById('totalAdditionalCargePrice');
                    if (totalChargePriceEl) {
                        totalChargePriceEl.textContent = '$' + grandTotal.toFixed(2);
                    }
                    grandAdditionalChargePrice = grandTotal;
                }
                ...
                document.addEventListener('input', function (e) {
                    if (e.target.matches('input[name="number_of_guests[]"]')) {
                        updateTotalGuests();
                        calculatePrices();
                    }
                });
                ...
                function calculatePrices() {
                    ...
                    finalPrice += transportPriceIn;
                    finalPrice += transportPriceOut;
                    finalPrice += grandAdditionalChargePrice;
                    ...
                    if (grandAdditionalChargePrice > 0) {
                        document.getElementById('totalAdditionalCarge').hidden = false;
                        document.getElementById('totalAdditionalCargeText').hidden = false;
                    }else{
                        document.getElementById('totalAdditionalCarge').hidden = true;
                        document.getElementById('totalAdditionalCargeText').hidden = true;
                    }
                }
