        ...
        <div class="pd-ltr-20">
            <div class="min-height-200px">
                ...
                <div class="row">
                            ...
                            @if (session('bookingcode'))
                                <div class="bookingcode-card">
                                    ...
                                    <div class="content-card">
                                        ...
                                        <div class="text-card">@lang('messages.Expired')
                                            {{ dateFormat(session('bookingcode.expired_date')) }}</div>
                                    </div>
                                    <div class="content-card-price">
                                        <div class="price"><span>$</span>{{ session('bookingcode.discounts') }}</div>
                                        <form id="removeBookingCode" action="{{ route('bookingcode.remove') }}"
                                            method="POST" style="display: inline;">
                                            @csrf
                                        </form>
                                        <button type="submit" form="removeBookingCode" class="btn-remove-code"><i
                                    ...
                                    <div class="card-box-body">
                                        <form id="searchHotelMobile" action="{{ route('view.hotel-prices', $hotel->code) }}"
                                            ...
                                            <div class="input-group-icon">
                                                ...
                                                <input readonly id="checkincout" name="checkincout"
                                                    class="form-control input-icon @error('checkincout') is-invalid @enderror"
                                                    type="text"
                                                    value="{{ dateFormat(session('booking_dates.checkin')) . ' - ' . dateFormat(session('booking_dates.checkout')) }}"
                                                    placeholder="@lang('messages.Check In') - @lang('messages.Check Out')" required>
                                                @error('checkincout')
                                        ...
                                        <div class="row">
                                            <div class="col-6 text-left">
                                                @if (session('booking_dates.duration') < $hotel->min_stay)
                                                    <p class="error-notification"> @lang('messages.Minimum stay') {{ $hotel->min_stay }}
                                                        @lang('messages.nights')</p>
                                                @endif
                                            </div>
                                            <div class="col-6">
                                                <button form="searchHotelMobile" type="submit" class="btn btn-primary"
                    ...
                    <div class="col-md-8">
                        @if (
                            (isset($error) && is_array($error) && count($error) > 0) ||
                                (isset($success) && is_array($success) && count($success) > 0))
                            ...
                            <div class="card-box-body">
                                        ...
                                        <ul class="nav nav-tabs" role="tablist">
                                            <li class="nav-item">
                                                <a class="nav-link active" data-toggle="tab" href="#detail"
                                                    role="tab">@lang('messages.Detail')</a>
                                            </li>
                                            @if ($hotel->facility)
                                                <li class="nav-item">
                                                    <a class="nav-link" data-toggle="tab" href="#facility-tab"
                                                        role="tab">@lang('messages.Facility')</a>
                                                </li>
                                            @endif
                                        </ul>
                                            ...
                                            <div class="tab-pane fade active show" id="detail" role="tabpanel">
                                                <div class="row">
                                                    ...
                                                    <div class="col-6 col-md-6">
                                                        <div class="content-title">@lang('messages.Location'):</div>
                                                        <p>
                                                            <a target="__blank"
                                                                href="{{ $hotel->map }}">{{ $hotel->region }}</a>
                                                        </p>
                                                    </div>
                                                    <div class="col-6 col-md-6">
                                                        <div class="content-title">@lang('messages.Address'):</div>
                                                        <p>{{ $hotel->address }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div class="card-box-content">
                                    @foreach ($hotel->rooms as $room)
                                        ...
                                        @endphp
                                        @include(
                                            'partials.hotel-room-card',
                                            compact('room', 'hotel', 'show_promo', 'promoImages', 'now'))
                                    @endforeach
                                </div>
                            </div>
                                ...
                                <div
                                    ...
                                    <div class="card-box-body">
                                        <form id="searchHotelDesktop"
                                            action="{{ route('view.hotel-prices', $hotel->code) }}" method="POST"
                                            role="search">
                                            {{ csrf_field() }}
                                            <div class="input-group-icon">
                                                <i class="icon-copy dw dw-calendar1"></i>
