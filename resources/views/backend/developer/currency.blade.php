...
@section('content')
    ...
    <main class="main-container currency-admin-page">
        <div class="pd-ltr-20">
            <x-backend.page-hero class="currency-admin-header">
                ...
                <x-slot name="copy">
                    <p>
                        Manage selling rates, buying rates, tax configuration, and payment bank accounts used by backend
            ...
            @if ($errors->any() || session()->has('success') || session()->has('error'))
                <section class="backend-feedback currency-admin-feedback">
                    ...
                    @if (session()->has('success'))
                        <div
                            class="backend-alert backend-alert--success currency-admin-alert currency-admin-alert--success">
                            <strong>Saved.</strong>
            ...
            <section class="backend-page-toolbar currency-admin-toolbar">
                ...
                <div class="currency-admin-toolbar__actions">
                    <div
                        class="currency-admin-source currency-admin-source--{{ $externalStatus === 'available' ? 'ok' : 'warning' }}">
                        <i
                            class="fa {{ $externalStatus === 'available' ? 'fa-check-circle' : 'fa-exclamation-circle' }}"></i>
                        <span>
                ...
                <div class="currency-admin-rates">
                    @foreach ($currencyRates as $rate)
                        @php
                            ...
                            $meta = $rateMeta[$code];
                            $spread =
                                is_numeric($rate['sell']) && is_numeric($rate['buy'])
                                    ? (float) $rate['sell'] - (float) $rate['buy']
                                    : null;
                                ...
                                @canany(['posDev', 'posAuthor'])
                                    @if ($rate['id'])
                                        <button type="button" class="backend-button backend-button-secondary"
                                            data-toggle="modal" data-target="#edit-rate-{{ $code }}">
                        ...
                        @canany(['posDev', 'posAuthor'])
                            @if ($rate['id'])
                                <div class="modal fade backend-modal currency-admin-modal" id="edit-rate-{{ $code }}"
                                    ...
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content">
                                            ...
                                            <form id="edit-rate-form-{{ $code }}"
                                                action="{{ route($routeName, $rate['id']) }}" method="post">
                                                @csrf
                                                @method('put')
                                                <div class="backend-modal__body currency-admin-modal__body">
                                                    ...
                                                    <div class="backend-form-grid currency-admin-form-grid">
                                                        <label>
                                                            ...
                                                            <input class="backend-form-control" name="sell" type="number"
                                                                step="0.01" min="0"
                                                                value="{{ old('sell', $rate['sell']) }}"
                                                                data-backend-money-unit="IDR" required>
                                                        </label>
                                                        <label>
                                                            <span>Spread / Difference <b>*</b></span>
                                                            <input class="backend-form-control" name="difference" type="number"
                                                                step="0.01" min="0"
                                                                value="{{ old('difference', $rate['difference']) }}"
                                                                data-backend-money-unit="IDR" required>
                                                        </label>
                                                    </div>
                                                    <p class="currency-admin-help">Buy rate is calculated automatically from
                                                        sell rate minus spread.</p>
                                                </div>
                ...
                <aside class="currency-admin-side">
                    <article class="currency-tax-card">
                        ...
                        @canany(['posDev', 'posAuthor', 'posAdm'])
                            @if ($tax)
                                <button type="button" class="backend-button backend-button-secondary" data-toggle="modal"
            ...
            <section class="backend-panel currency-bank-section">
                <div class="backend-section-header currency-admin-section-heading m-b-18">
                    <div>
                                ...
                                @canany(['posDev', 'posAuthor'])
                                    <div class="currency-bank-card__actions">
                                        <button type="button" data-toggle="modal"
                                            data-target="#edit-bank-account-{{ $bank->id }}" title="Edit bank account">
                                            <i class="fa fa-pencil-alt"></i>
                                        </button>
                                        <form action="/delete-bank-account/{{ $bank->id }}" method="post"
                                            data-confirm-delete="Delete {{ $bank->bank }} account?">
                                            @csrf
                                            @method('delete')
    ...
    @canany(['posDev', 'posAuthor', 'posAdm'])
        @if ($tax)
            <div class="modal fade backend-modal currency-admin-modal" id="edit-tax" tabindex="-1" role="dialog"
                aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="backend-modal__header currency-admin-modal__header">
                        ...
                        <form id="edit-tax-form" action="{{ route('f-update-tax', $tax->id) }}" method="post">
                            ...
                            <div class="backend-modal__body currency-admin-modal__body">
                                <label class="currency-admin-field">
                                    ...
                                    <input class="backend-form-control" name="tax" type="number" step="0.01"
                                        min="0" value="{{ old('tax', $tax->tax) }}" required>
                                </label>
                            </div>
                            <div class="backend-modal__footer currency-admin-modal__footer">
                                <button type="submit" class="backend-button backend-button-primary">
        ...
        @else
            <div class="modal fade backend-modal currency-admin-modal" id="add-tax" tabindex="-1" role="dialog"
                aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="backend-modal__header currency-admin-modal__header">
                        ...
                        <form id="edit-tax-form" action="{{ route('f-add-tax') }}" method="post">
                            @csrf
                            @method('put')
                            <div class="backend-modal__body currency-admin-modal__body">
                                <label class="currency-admin-field">
