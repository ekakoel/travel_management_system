...
@section('content')
    @canany(['posDev','posAuthor','posAdm'])
        ...
        <main class="main-container hotel-form-page hotel-form-page--create">
            <div class="pd-ltr-20">
                <x-backend.page-hero
                    class="hotel-form-hero"
                    eyebrow="Hotel Profile"
                    title="Add Hotel"
                    description="Create a Hotel master profile before rooms, contracts, normal prices, promos, and packages are configured."
                >
                    <x-slot name="action">
                ...
                <form id="hotelCreateForm" class="backend-form" action="{{ route('admin.hotel.store') }}" method="post" enctype="multipart/form-data">
                    @csrf

                    <x-backend.detail-layout class="hotel-create-layout">
                        <x-slot name="main">
                            <section class="backend-panel backend-form-panel hotel-form-panel">
                                <div class="backend-section-header">
                                    ...
                                    <p>Upload the primary image used for the Hotel profile and public listing.</p>
                                </div>
                                    ...
                                    <div class="backend-form-grid">
                                        <div class="backend-form-field is-wide">
                                            ...
                                            <div class="hotel-form-cover-control">
                                                <figure class="hotel-form-cover-preview" data-hotel-cover-preview></figure>
                                                <div class="hotel-form-cover-input">
                                                    <input type="file" name="cover" id="cover" class="backend-form-control @error('cover') is-invalid @enderror" accept="image/jpeg,image/png,image/jpg,image/webp" data-hotel-cover-input data-hotel-cover-preview-target="[data-hotel-cover-preview]" required>
                                                    <span class="hotel-file-status" data-hotel-cover-status data-hotel-cover-status-default="No cover selected">No cover selected</span>
                                                    @error('cover')
                                ...
                                <div class="backend-form-panel__body">
                                    <div class="backend-form-grid">
                                        ...
                                        <div class="backend-form-field">
                                            ...
                                            <input type="text" inputmode="tel" id="phone" name="phone" class="backend-form-control @error('phone') is-invalid @enderror" placeholder="Insert contact person phone" value="{{ old('phone') }}" required>
                                            @error('phone')
                                                <span class="invalid-feedback d-block">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="backend-form-field">
                                            ...
                                            <input type="text" id="web" name="web" class="backend-form-control @error('web') is-invalid @enderror" placeholder="Ex: www.example.com" value="{{ old('web') }}" required>
                                            @error('web')
                                                <span class="invalid-feedback d-block">{{ $message }}</span>
                                            @enderror
                                        </div>
                                ...
                                <div class="backend-form-panel__body">
                                    <div class="backend-form-grid">
                                        <div class="backend-form-field">
                                            ...
                                            <input type="number" min="0" id="min_stay" name="min_stay" class="backend-form-control @error('min_stay') is-invalid @enderror" placeholder="Minimum stay" value="{{ old('min_stay') }}">
                                            @error('min_stay')
                                                <span class="invalid-feedback d-block">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="backend-form-field">
                                            ...
                                            <input type="number" min="0" id="max_stay" name="max_stay" class="backend-form-control @error('max_stay') is-invalid @enderror" placeholder="Maximum stay" value="{{ old('max_stay') }}">
                                            @error('max_stay')
                                                <span class="invalid-feedback d-block">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="backend-form-field">
                                            <label for="airport_distance" class="backend-form-label">Airport Distance (Hours) <span>*</span></label>
                                        ...
                                        <div class="backend-form-field">
                                            <label for="airport_duration" class="backend-form-label">Airport Duration (Km) <span>*</span></label>
                                            ...
                                            <div class="backend-translation-grid">
                                                @foreach ($group['fields'] as $field)
                                                    <div class="backend-translation-field">
                                                        ...
                                                        <textarea id="{{ $field['name'] }}" name="{{ $field['name'] }}" class="textarea_editor backend-form-control border-radius-0 @error($field['name']) is-invalid @enderror" data-backend-richtext="true" placeholder="{{ $field['placeholder'] }}" @if ($field['required']) required @endif>{{ old($field['name']) }}</textarea>
                                                        @error($field['name'])
                                                            <span class="invalid-feedback d-block">{{ $message }}</span>
                                                        @enderror
                                                    </div>
                        ...
                        <x-slot name="side">
                            <section class="backend-panel backend-detail-side-card hotel-create-context-panel hotel-status-side-card">
                                <div class="backend-section-header">
                                    ...
                                    <p>The server creates new Hotels as Draft. Status is not accepted from this form.</p>
                                </div>
                            </section>
                            ...
                            <section class="backend-panel backend-detail-side-card hotel-create-context-panel">
                                <div class="backend-section-header">
                                    <div>
                                        <span class="backend-section-header__label">Hotel Setup Guidance</span>
                                        <h2>Before Save</h2>
                                    </div>
                                    <p>Administrative reminders for creating the Hotel master record.</p>
                                </div>
                                <ul class="backend-detail-side-list">
                                    <li>
                                        <span>Profile</span>
                                        <strong>Complete property basics first</strong>
                                        <small>Name, region, address, contact, website, and cover are required before the Hotel can be created.</small>
                                    </li>
                                    <li>
                                        <span>Rooms</span>
                                        <strong>Room setup happens after creation</strong>
                                        <small>Add room inventory from the Hotel detail page once the Hotel profile exists.</small>
                                    </li>
                                    <li>
                                        <span>Pricing</span>
                                        <strong>Rates are managed separately</strong>
                                        <small>Normal prices, promos, packages, contracts, and additional charges are configured after this record is saved.</small>
                                    </li>
                                </ul>
                            </section>
                            ...
                            <section class="backend-panel backend-detail-side-card hotel-create-context-panel">
                                <div class="backend-section-header">
                                    <div>
                                        <span class="backend-section-header__label">Stay Rules</span>
                                        <h2>Minimum and Maximum Stay</h2>
                                    </div>
                                </div>
                                <ul class="backend-detail-side-list">
                                    <li>
                                        <span>Minimum Stay</span>
                                        <strong>Lowest accepted stay length</strong>
                                        <small>Leave empty only when the Hotel has no master minimum stay rule.</small>
                                    </li>
                                    <li>
                                        <span>Maximum Stay</span>
                                        <strong>Upper stay guidance</strong>
                                        <small>When filled, maximum stay must be greater than or equal to minimum stay.</small>
                                    </li>
                                </ul>
                            </section>
                            ...
                            <section class="backend-panel backend-detail-side-card hotel-create-context-panel">
                                <div class="backend-section-header">
                                    <div>
                                        <span class="backend-section-header__label">Next Step Context</span>
                                ...
                                <ul class="backend-detail-side-list">
                                    <li>
                                        <span>Media</span>
                                        <strong>Add gallery from detail</strong>
                                        <small>The cover is uploaded now; gallery images are managed after the Hotel exists.</small>
                                    </li>
                                    <li>
                                        <span>Publish</span>
                                        <strong>Review before activation</strong>
                                        <small>Keep the Hotel Draft until rooms and pricing are ready for public booking.</small>
                                    </li>
                                </ul>
                            </section>
                        </x-slot>
                    ...
                    <section class="backend-page-toolbar backend-form-actions hotel-form-actions">
                        <div class="backend-page-toolbar__actions">
                            <a href="{{ route('admin.hotels.index') }}" class="backend-button backend-button-secondary">Cancel</a>
