        ...
        @uiEnabled('doku-payment')
            @if ($doku_payment)
                <div class="order-detail-note mt-3">
                    <strong>@lang('messages.Pay with DOKU')</strong>
                    <div class="order-detail-price-row mt-2">
                        <span>@lang('messages.Total') IDR</span>
                        <strong>{{ currencyFormatIdr($total_price_idr) }}</strong>
                    </div>
                    <div class="order-detail-price-row">
                        <span>@lang('messages.Payment Fee')</span>
                        <strong>{{ currencyFormatIdr($tax_doku) }}</strong>
                    </div>
                    <div class="order-detail-price-row order-detail-price-row--grand">
                        <span>@lang('messages.Total Price') IDR</span>
                        <strong>{{ currencyFormatIdr($doku_total_price) }}</strong>
                    </div>
                </div>
            @endif
        @endUiEnabled
