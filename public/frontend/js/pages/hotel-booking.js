(function ($) {
    ...
    function formatNativeDate(value) {
        ...
        if (typeof moment !== 'undefined') {
            var parsed = moment(value, ['YYYY-MM-DD', 'Y-m-d', 'F j, Y', moment.ISO_8601], true);
            if (parsed.isValid()) {
    ...
    function formatNativeDateTime(value) {
        ...
        if (typeof moment !== 'undefined') {
            var parsed = moment(value, [moment.ISO_8601, 'YYYY-MM-DDTHH:mm', 'YYYY-MM-DD H:mm', 'YYYY-MM-DD HH:mm:ss', 'Y-m-d H:i', 'F j, Y h:i K'], true);
            if (parsed.isValid()) {
                return parsed.format('MM/DD/YYYY hh:mm A');
            }
        }
