...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function getCurrentDuration() {
    var durationSource = durationField && !durationField.closest('[hidden]') ? durationField.value : durationInput ? durationInput.value : '';
  ...
  function calculateBaseTotal() {
    ...
    return selectedRate.type === 'Daily Rent' ? selectedRate.priceValue * getCurrentDuration() : selectedRate.priceValue;
  }
  ...
  function renderReviewGuestRows(guestEntries) {
    ...
    guestEntries.forEach(function (guest, index) {
      ...
      var values = [index + 1, guest.name || '-', guest.age || '-', guest.sex || '-', guest.phone || '-'];
  ...
  function isFieldVisible(field) {
    ...
    return !field.disabled && !field.closest('[hidden]') && field.offsetParent !== null;
  }
  ...
  function validateServiceStep(options) {
    ...
    for (var i = 0; i < requiredFields.length; i += 1) {
      ...
      if (!field || !isFieldFilled(field) || !field.checkValidity()) {
        if (shouldNotify) {
          var message = field && field.id === 'flight_date' ? 'Please select the flight date before continuing to guest details.' : field && field.id === 'transportAgent' ? 'Please select an agent before continuing.' : flightRequiredValidationMessage;
  ...
  function updateTripReviewButtonState() {
    ...
    var isStepValid = validateTripAndGuestsStep({
      notify: false
    });
    ...
    if (durationField) {
      if (isDailyRent) {
        if (!durationField.value || parseInt(durationField.value, 10) < 1) {
          durationField.value = durationInput && parseInt(durationInput.value, 10) > 0 ? durationInput.value : '1';
        }
      }
    }
    if (shuttleTypeGroup) {
  ...
  function renderSelectedRate(rate) {
    ...
    if (modalDurationLabelTarget) {
      modalDurationLabelTarget.textContent = rate.type === 'Daily Rent' ? estimatedRentalDurationLabel : estimatedDurationLabel;
    }
    if (modalExtraTarget) {
    ...
    if (reviewFlightType) {
      reviewFlightType.textContent = isAirportShuttle ? shuttleTypeSelect && shuttleTypeSelect.value || oldShuttleType || 'Arrival' : '-';
    }
    if (reviewFlightNumber) {
      reviewFlightNumber.textContent = isAirportShuttle ? flightNumberInput && flightNumberInput.value.trim() || '-' : '-';
    }
    if (reviewPickupLocation) {
      reviewPickupLocation.textContent = isDailyRent ? pickupLocationInput && pickupLocationInput.value.trim() || rate.src || '-' : '-';
    }
    if (reviewDropoffLocation) {
      reviewDropoffLocation.textContent = isDailyRent ? dropoffLocationInput && dropoffLocationInput.value.trim() || rate.dst || '-' : '-';
    }
    if (modalRateNote) {
  ...
  function selectFirstRateByType(type, preferredDestination) {
    ...
    var destination = type === 'Airport Shuttle' ? preferredDestination || (sidebarDestinationSelect ? sidebarDestinationSelect.value : '') : '';
  ...
  function createGuestItem(index) {
    ...
    wrapper.innerHTML = '<div class="transport-reservation-guest-item__index">' + '<strong data-transport-guest-label>' + escapeHtml(getGuestLabel(index)) + '</strong>' + '</div>' + '<div class="transport-reservation-guest-item__content">' + '<div class="transport-reservation-field transport-reservation-field--compact">' + '<label>' + escapeHtml(labelName) + ' <span class="transport-reservation-required" aria-hidden="true">*</span></label>' + '<input type="text" name="guest_entries[' + index + '][name]" class="form-control" placeholder="' + escapeHtml(placeholderGuestName) + '" required>' + '</div>' + '<div class="transport-reservation-field transport-reservation-field--compact">' + '<label>' + escapeHtml(labelAgeCategory) + ' <span class="transport-reservation-required" aria-hidden="true">*</span></label>' + '<select name="guest_entries[' + index + '][age]" class="form-control" required>' + '<option value="Adult" selected>' + escapeHtml(adultLabel) + '</option>' + '<option value="Child">' + escapeHtml(childLabel) + '</option>' + '</select>' + '</div>' + '<div class="transport-reservation-field transport-reservation-field--compact">' + '<label>' + escapeHtml(labelGender) + ' <span class="transport-reservation-required" aria-hidden="true">*</span></label>' + '<select name="guest_entries[' + index + '][sex]" class="form-control" required>' + '<option value="">' + escapeHtml(selectGenderLabel) + '</option>' + '<option value="Male">' + escapeHtml(maleLabel) + '</option>' + '<option value="Female">' + escapeHtml(femaleLabel) + '</option>' + '</select>' + '</div>' + '<div class="transport-reservation-field transport-reservation-field--compact">' + '<label>' + escapeHtml(labelPhone) + ' <span class="transport-reservation-optional">(' + escapeHtml(labelOptional) + ')</span></label>' + '<input type="text" name="guest_entries[' + index + '][phone]" class="form-control" placeholder="' + escapeHtml(placeholderPhone) + '">' + '</div>' + '<div class="transport-reservation-guest-item__action">' + '<label class="transport-reservation-guest-item__action-label">&nbsp;</label>' + '<button type="button" class="transport-reservation-guest-item__remove" data-remove-transport-guest>' + '<span aria-hidden="true">X</span>' + '</button>' + '</div>' + '</div>';
    return wrapper;
  }
  page.querySelectorAll('[data-transport-datetime]').forEach(function (input) {
    try {
      ...
      if (window.jQuery && typeof window.jQuery.fn.daterangepicker === 'function' && typeof window.moment === 'function') {
        ...
        var resolveTransportPickerDrops = function resolveTransportPickerDrops(picker) {
  ...
  ['#flight_date', '#flight_number', '#pickup_location', '#dropoff_location'].forEach(function (selector) {
    ...
    field.addEventListener('input', function () {
      clearInlineFieldError(field);
      if (selector === '#flight_date' || selector === '#flight_number') {} else {
        updateTripReviewButtonState();
      }
      if (!selectedRate) {
        return;
  ...
  if (modalForm) {
    var initialNavigation = window.performance && window.performance.getEntriesByType ? window.performance.getEntriesByType('navigation')[0] : null;
