...
document.addEventListener('DOMContentLoaded', function () {
  var page = document.querySelector('[data-tour-packages-page]');
  ...
  function getForm() {
    return page.querySelector('[data-tour-packages-filter-form]');
  }
  function getResults() {
    return page.querySelector('[data-tour-packages-results]');
  }
  ...
  function replaceFromDocument(doc) {
    var nextSummary = doc.querySelector('[data-tour-packages-summary]');
    var currentSummary = page.querySelector('[data-tour-packages-summary]');
    var nextForm = doc.querySelector('[data-tour-packages-filter-form]');
    var currentForm = getForm();
    var nextResults = doc.querySelector('[data-tour-packages-results]');
  ...
  page.addEventListener('submit', function (event) {
    var form = event.target.closest('[data-tour-packages-filter-form]');
    if (!form) {
      return;
    }
  ...
  page.addEventListener('input', function (event) {
    if (!event.target.matches('[data-tour-packages-search]')) {
      return;
    }
  ...
  page.addEventListener('change', function (event) {
    if (!event.target.matches('[data-tour-packages-filter]')) {
      return;
    }
    var form = getForm();
    if (form) {
  ...
  page.addEventListener('click', function (event) {
    var resetButton = event.target.closest('[data-tour-packages-reset]');
    var paginationLink = event.target.closest('[data-tour-packages-results] .pagination a');
    if (resetButton) {
      event.preventDefault();
