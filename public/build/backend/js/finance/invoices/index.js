/******/ (() => { // webpackBootstrap
/*!********************************************************!*\
  !*** ./resources/backend/js/finance/invoices/index.js ***!
  \********************************************************/
(function () {
  'use strict';
  ...
  function applyFilters() {
    ...
    rows.forEach(function (row) {
      var matches = (row.dataset.invoiceSearch || '').includes(search) && (!state || row.dataset.invoiceState === state) && (!currency || row.dataset.invoiceCurrency === currency) && (!due || row.dataset.invoiceDue === due);
      row.hidden = !matches;
